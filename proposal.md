...
The Branch Pruning Pass speculatively filter out unused or infrequently used
basic blocks, based on the branch probabilities converted from the circular buffer.
Unlike Unreachable Code Elimination, basick blocks that filtered out by
the branch pruning pass are not actually removed from the MIRGraph.
Instead, we reuse the basic blocks as containers for bailout instructions.
...
Inplement a basic block counter version if the overhead keeps high. (3 weeks)
